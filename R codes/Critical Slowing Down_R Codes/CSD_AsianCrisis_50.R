...
start <- which(time(data) == '1997-10-17') - 100
...
p1 <- ggplot(data = NULL, mapping = aes(x = -100:100, y = data)) +
  ...
  geom_segment(aes(x = -100, y = 9.07, xend = -50, yend = 9.07), arrow = arrow(length = unit(0.2, 'cm'))) +
  geom_segment(aes(x = -50, y = 9.07, xend = -100, yend = 9.07), arrow = arrow(length = unit(0.2, 'cm'))) +
  xlab("") +
  ylab("") +
  theme_bw() +
  ...
  annotate("text", label = "Sliding window", x = -75, y = 9.13,
           fontface = 2, size = 3) +
  geom_label(aes(x = 98.1, y = 9.71, fontface = 2),
             label = 'HSI', fill = 'grey')
...
p2 <- ggplot(data = NULL, mapping = aes(x = -100:100, y = residuals)) +
  ...
  theme(axis.text.x = element_text(face = 'bold', colour = 'black', size = 10),
        axis.text.y = element_text(face = 'bold', colour = 'black', size = 9, angle = 45)) +
  scale_x_discrete(limits = c(-100, -75, -50, -25, 0, 50, 100)) +
  geom_label(aes(x = 93.7, y = 0.05, fontface = 2),
...
p3 <- ggplot(data = NULL, mapping = aes(x = -100:100, y = AR)) +
  ...
  theme(axis.text.x = element_text(face = 'bold', colour = 'black', size = 10),
        axis.text.y = element_text(face = 'bold', colour = 'black', size = 9, angle = 45)) +
  scale_x_discrete(limits = c(-100, -75, -50, -25, 0, 50, 100)) +
  geom_label(aes(x = 96.49, y = 0.923, fontface = 2),
             label = 'AR(1)', fill = 'grey') +
  geom_label(aes(x = - 79.8, y = 0.59),
...
p4 <- ggplot(data = NULL, mapping = aes(x = -100:100, y = sqrt(variance))) +
  ...
  scale_x_discrete(limits = c(-100, -75, -50, -25, 0, 50, 100)) +
  geom_label(aes(x = 94.9, y = 0.029, fontface = 2),
             label = 'Std. dev', fill = 'grey') +
  geom_label(aes(x = - 80, y = 0.0154),
...
png('CSD_AsianCrisis_50.png', width = 900, height = 600)
grid.newpage()
grid.draw(g)
dev.off()
