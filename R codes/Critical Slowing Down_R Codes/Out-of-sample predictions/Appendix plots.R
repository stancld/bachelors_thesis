...
getSymbols('^DJI', env = .GlobalEnv, from = '2003-03-15', to = '2008-02-17', periodicity = 'daily')
...
##### Period1
start <- 1
end <- which(time(data) == '2007-09-17')
...
###### PLOTS
p1 <- ggplot(data = NULL, mapping = aes(x = decimal_date(time(data)), y = data$DJI.Adjusted)) +
  ...
  scale_y_continuous(limits = c(8.95, 9.6), expand = c(0,0)) +
  theme_gray() +
...
geom_label(aes(x = max(decimal_date(time(data))) - 0.08, y = 9.576, fontface = 2),
...
p2 <- ggplot(data = NULL, mapping = aes(x = decimal_date(time(data)), y = residuals)) +
  ...
  scale_x_continuous(limits = c(min(decimal_date(time(data))),
                                max(decimal_date(time(data)))), expand = c(0,0)) +
  geom_label(aes(x = max(decimal_date(time(data))) - 0.155, y = 0.035, fontface = 2),
...
p3 <- ggplot(data = NULL, mapping = aes(x = decimal_date(time(data)), y = AR)) +
  ...
  scale_x_continuous(limits = c(min(decimal_date(time(data))),
                                max(decimal_date(time(data)))), expand = c(0,0)) +
  geom_label(aes(x = max(decimal_date(time(data))) - 0.09, y = 0.711, fontface = 2),
...
p4 <- ggplot(data = NULL, mapping = aes(x = decimal_date(time(data)), y = sqrt(variance))) +
  ...
  scale_x_continuous(limits = c(min(decimal_date(time(data))),
                                max(decimal_date(time(data)))), expand = c(0,0)) +
  geom_label(aes(x = max(decimal_date(time(data))) - 0.138, y = 0.1, fontface = 2),
...
png('crisis2007_1.png', width = 900, height = 600)
grid.newpage()
grid.draw(g)
dev.off()
