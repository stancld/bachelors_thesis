...
start <- which(time(data) == '1987-10-16') - 200
...
p4 <- ggplot(data = NULL, mapping = aes(x = -200:100, y = sqrt(variance))) +
  ...
  geom_label(aes(x = - 170, y = 0.01525),
...
start <- which(time(gspc) == '1987-10-16') - 200
...
for (t in time_frame){
  MA.gspc[t] <- sum(sapply(time_frame, function(x) gaussian_kernel(sigma = 10, r = x, t = t) * DATA[x,1])) /
    sum(sapply(time_frame, function(x) gaussian_kernel(sigma = 10, r = x, t = t)))
}

residuals.gspc <- DATA[1:200,1] - MA.gspc[1:200]
plot(residuals.gspc)
...
residuals.n225 <- DATA[1:200,2] - MA.n225[1:200]
plot(residuals.n225)
...
residuals.ftse <- DATA[1:200,3] - MA.ftse[1:200]
plot(residuals.ftse)

  # 2. computing correlations
gspcXnikkei <- rep(NA, 100)
for (i in 1:100){
  end <- i + 99
  gspcXnikkei[i] <- cor(x = residuals.gspc[i:end],
                     y = residuals.n225[i:end])
}
gspcXnikkei <- c(rep(NA, 100), gspcXnikkei, rep(NA, 101))

gspcXftse <- rep(NA, 100)
for (i in 1:100){
  end <- i + 99
  gspcXftse[i] <- cor(x = residuals.gspc[i:end],
                        y = residuals.ftse[i:end])
}
gspcXftse <- c(rep(NA, 100), gspcXftse, rep(NA, 101))
...
png('CSD_BlackMonday873.png', width = 900, height = 750)
grid.newpage()
grid.draw(g)
dev.off()