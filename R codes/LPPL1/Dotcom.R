...
base_data <- IXIC$IXIC.Adjusted
plot(log(base_data))
base_data$IXIC.Adjusted <- log(base_data$IXIC.Adjusted)

end <- end <- which(time(base_data) == '2000-03-10') - 50
base_data <- base_data[1:end,]
plot(base_data$price)
...
head(base_data)
tail(base_data)
write.csv(base_data, 'dotcom.csv')
# Defining functions ------------------------------------------------------
...
TabuSearch <- function(A, B, C){
  ...
  t_c_set <- runif(n = 40, min = max(base_data$t), max = max(base_data$t) + 0.65) # adding 0.6 year represent time horizon within the crash is expected
...
grid <- list(a = seq(8.3, 8.7, length.out = 5),
...
data <- data %>%
  filter(m < 0.6)
write.csv(data, 'DotCom_TS.csv')
TOP15 <- head(data, 15)
write.csv(TOP15, 'Dotcom10.csv')

# OLS ---------------------------------------------------------------------
(ABC <- read.csv('ABCD.csv'))
