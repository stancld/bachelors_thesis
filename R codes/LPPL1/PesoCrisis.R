...
grid <- list(a = seq(7.6, 8, length.out = 5),
...
data <- data %>%
  arrange(loss)
data
write.csv(data, 'PesoCrisis_TS.csv')
head(data, 10)
...
data2 <- log(MXX$MXX.Adjusted[1:end])
(data2$time <- decimal_date(time(data2)))
for (i in 1:length(data2)){
  if (is.na(data2$MXX.Adjusted[i]) == T){
    data2$MXX.Adjusted[i] <- data2$MXX.Adjusted[i-1]
  }
}
plot(data2$MXX.Adjusted)
head(data2)
...
# Plotting ----------------------------------------------------------------
plot(as.numeric(data2$MXX.Adjusted), type = 'l')
...
##### TOP PLOTTING
p2 <- ggplot(data = NULL, mapping = aes(x = decimal_date(time(data2)), y = data2$MXX.Adjusted)) +
  geom_line(size = 0.8) +
  ...
  geom_vline(xintercept = decimal_date(time(data2[which.max(data2$MXX.Adjusted)])), linetype = 'dashed', size = 0.7, col = 'black') +
  geom_vline(xintercept = 1993.97, linetype = 'dotted', size = 0.8, col = 'black') +
  ylab("") +
  xlab('Time (years)') +
  scale_y_continuous(limits = c(7.3, 8), expand = c(0,0)) +
  ...
  geom_label(aes(x = 1993.455, y = 7.983), fontface = 2,
             label = 'MXX', fill = 'grey')

p2

png('LPPL1_PesoCrisis.png', width = 900, height = 300)
p2
dev.off()
